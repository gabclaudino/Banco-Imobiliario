...
public class Tela extends JFrame implements ActionListener {
    ...
    public void iniciaInterface(){
        ...
        for(Jogador jogador : tabuleiro.getJogadores()) {
            tabuleiro.getCasas().getFirst().adicionarJogador(jogador);
        }
        ...
        for (int i = 0; i < tabuleiro.getCasas().size()/2; i++)
        {
            if(tabuleiro.getCasas().get(i).getTipo().equalsIgnoreCase("TERRENO")){
                tabuleiro.getCasas().get(i).setDono(tabuleiro.getJogadores().getFirst());
                tabuleiro.getJogadores().getFirst().adicionarPropriedade(tabuleiro.getCasas().get(i));
            }
        }
    ...
    public void itera(){
        ...
        if(tabuleiro.getJogadores().size() == 1) {
            JOptionPane.showMessageDialog(this, "O jogo acabou e o vencedor foi "+tabuleiro.getJogadores().getFirst()+"! Parabéns!");
            System.exit(0);
        }
